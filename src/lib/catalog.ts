...
// Import catalog JSON from src/data
...
/**
 * Search only across pattern.name, pattern.definition, pattern.goal
 * (per spec: narrowed search scope)
 */
function matchesSearchPattern(p: Pattern, q: string): boolean {
  if (!q) return true;
  const haystack = [p.name, p.definition, p.goal].join(" ").toLowerCase();
  return haystack.includes(q);
}

/**
 * Returns true if haystack includes ALL items in needles
 */
 ...
 * 
 * Client-safe: uses direct JSON import (no Node fs dependency)
 */
export function loadCatalog(): CatalogAccess {
  ...
  /**
   * Filter function with AND semantics:
   * - Match ALL selected within each facet (gdprTags, uiEvidenceTags, heuristics)
   * - AND across all facets
  ...
  function applyFilters(filters: FilterState): FilterResult {
    ...
    const patterns = catalog.patterns
      ...
      // Sort alphabetically by name (always)
